...
$JSON = mysqli_real_escape_string($conn, $JSON);
 ...
 if(!$conn){
  ...
  $sql = "CREATE TABLE IF NOT EXISTS `TableJointure` (`StudentID` VARCHAR(255) NOT NULL, `ClassID` VARCHAR(255) NOT NULL, `ExamID`             VARCHAR(255) NOT NULL, `JSON` TEXT(4294967295), CONSTRAINT `uniqueAttributes` UNIQUE (StudentID, `ClassID`, `ExamID`, JSON(255))) CHARSET =  utf8";
  if ($conn->query($sql) === TRUE) {
  //  echo "Table created successfully <br>";
  } else {
    echo "Error creating table: <br>" . $conn->error;
  ...
  $sql = "INSERT INTO TableJointure (StudentID, ClassID, ExamID, JSON) VALUES ('$StudentID', '$ClassID', '$ExamID', '$JSON')";
  if ($conn->query($sql) === TRUE) {
  //  echo "Informations inserted successfully <br>";
  } else {
  //  echo "Error inserting informations: <br>" . $conn->error;
  ...
  ?>
      ...
      <ul>
         ...
         <li><a class="t1" id="up" href="#Importer">Importer</a>
             <ul id="down">
         ...
         <li style="float:right">
             <c class="container t4bis fas fa-moon">
                 <label class="switch">
                     <input type="checkbox">
